...
title.Text = "PolariaX (Version: Beta 1.1)"
...
-- Infinite Yield
makeBtn(fe,"Infinite Yield",20,function()
	PolariaX_InfiniteYield()
end)
...
-- KICK PLAYER GUI
--==============================
makeBtn(fe,"Kick Player GUI",100,function()

	local player = game.Players.LocalPlayer

	-- Prevent duplicates
	if player.PlayerGui:FindFirstChild("KickGui") then
		player.PlayerGui.KickGui:Destroy()
	end

	local guiKick = Instance.new("ScreenGui", player.PlayerGui)
	guiKick.Name = "KickGui"
	guiKick.ResetOnSpawn = false

	local frame = Instance.new("Frame", guiKick)
	...
	box.Position = UDim2.fromOffset(10,60)
	box.TextSize = 18
	...
	kickBtn.Position = UDim2.fromOffset(10,110)
	...
	kickBtn.MouseButton1Click:Connect(function()
		...
		if target then
			target:Kick("You have been kicked from the game.")
		end
	end)
end)
...
-- KILL PLAYER GUI
--==============================
makeBtn(fe,"Kill Player GUI",140,function()
	...
	if player.PlayerGui:FindFirstChild("KillGui") then
		player.PlayerGui.KillGui:Destroy()
	end
	...
	blackFrame.Size = UDim2.fromOffset(200,50)
	blackFrame.Position = UDim2.new(0.5,-100,0.2,0)
	...
	-- Close
	local close = Instance.new("TextButton", blackFrame)
	close.Size = UDim2.fromOffset(40,50)
	...
	grayFrame.Size = UDim2.fromOffset(200,100)
	grayFrame.Position = blackFrame.Position + UDim2.fromOffset(0,50)

	-- TextBox
	local box = Instance.new("TextBox", grayFrame)
	box.PlaceholderText = "Player Name"
	...
	-- Kill button
	local killBtn = Instance.new("TextButton", grayFrame)
	killBtn.Text = "Kill Player"
	killBtn.Size = UDim2.new(1,0,0.4,0)
	...
	-- Drag system (both frames)
	local dragging = false
	local dragStart, startTop, startMain

	blackFrame.InputBegan:Connect(function(i)
		if i.UserInputType == Enum.UserInputType.MouseButton1 then
			dragging = true
			dragStart = i.Position
			startTop = blackFrame.Position
			startMain = grayFrame.Position
		end
	end)
	...
	UIS.InputChanged:Connect(function(i)
		if dragging and i.UserInputType == Enum.UserInputType.MouseMovement then
			...
			blackFrame.Position = startTop + UDim2.fromOffset(delta.X,delta.Y)
			grayFrame.Position = startMain + UDim2.fromOffset(delta.X,delta.Y)
		end
	end)
